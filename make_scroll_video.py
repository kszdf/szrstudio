...
# v2 增强：加大情绪区分度（emphasis 更快更响、warn 更压、query 更升），去"安全值"导致的平铺感
EMOTION_PROSODY = {
    "narrate":  {"rel": (1.00, 1.00, 1.00), "pause": 0.20},  # 平铺叙述、交代背景
    "emphasis": {"rel": (1.18, 1.06, 1.15), "pause": 0.45},  # 重点强调、核心结论（明显提速加大音量）
    "warn":     {"rel": (1.08, 0.94, 1.05), "pause": 0.42},  # 风险警示、提醒（略快、明显压低音高）
    "query":    {"rel": (1.10, 1.06, 1.03), "pause": 0.40},  # 反问、抛问、悬念（略快、明显升调）
    "light":    {"rel": (1.12, 1.06, 0.92), "pause": 0.22},  # 轻松调侃、缓和（略快、升调、略轻）
    "ending":   {"rel": (1.00, 0.98, 0.96), "pause": 0.35},  # 收尾落点、总结（放慢、降调、音量回落）
}
_EMOTION_CACHE = {}
...
def naturalize_dialogue(text):
    ...
    prompt = (
        ...
        "3. 语气词按语义自然适配（不是全禁、也不滥用）：疑问/反问句尾可用「呢/吗」收升调（如「那这种情况税务局会怎么认定呢？」）；求证/确认句尾可用「吧」收降调（如「老板用个人卡收货款，风险不小吧？」）；提醒/警示句首可用「注意/记住」、句尾可带「啊」加重（如「注意啊，个人卡收款会被盯上」）；举例可用「比方说吧」；每句最多 1 个语气词；严禁堆「嗯、哦、对的、是的呢、哎哟、好家伙、对喽、说白了、对吧」等口头禅式承接；独白（单声）几乎不用语气词，靠停顿和语速变化制造自然感；老张是实战派，说话干脆、直给、不拖长音；\n"
