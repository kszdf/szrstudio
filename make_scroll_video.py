...
# v3 收敛：①音高起伏收窄到 ±2%（原 query 1.06 vs warn 0.94 相邻交替=忽高忽低）；
#          ②query 语速放缓到 1.02（提问句不该快，女声基准0.98×1.02≈1.0 自然）；
#          ③warn 不再压低音高（0.94→0.99，男声基准0.95 再压=浑浊），改靠音量+停顿警示。
EMOTION_PROSODY = {
    "narrate":  {"rel": (1.00, 1.00, 1.00), "pause": 0.20},  # 平铺叙述、交代背景
    "emphasis": {"rel": (1.12, 1.02, 1.12), "pause": 0.45},  # 重点强调、核心结论（提速加大音量，音高微升）
    "warn":     {"rel": (1.04, 0.99, 1.06), "pause": 0.42},  # 风险警示（略快、音量加重、音高几乎不变→不闷）
    "query":    {"rel": (1.02, 1.02, 1.02), "pause": 0.40},  # 反问、抛问（语速微快、音高微升，不抢话）
    "light":    {"rel": (1.06, 1.03, 0.94), "pause": 0.22},  # 轻松调侃、缓和（略快、略升、略轻）
    "ending":   {"rel": (0.98, 0.99, 0.96), "pause": 0.35},  # 收尾落点（放慢、音高微降、音量回落）
}
_EMOTION_CACHE = {}
...
def naturalize_dialogue(text):
    ...
    prompt = (
        ...
        "3. 语气词按语义自然适配、适度放开口语感（不是全禁、也不滥用）：疑问/反问句尾用「呢/吗」收升调（如「那这种情况税务局会怎么认定呢？」）；求证/确认句尾用「吧」收降调（如「老板用个人卡收货款，风险不小吧？」）；提醒/警示句首用「注意/记住」、句尾可带「啊/哈」加重（如「注意啊，个人卡收款会被盯上」）；举例用「比方说吧」「比如说」；承接可用「那」「其实」「你看」「怎么说呢」「这样吧」等轻口头语自然过渡（如「那要真被查到了呢？」「其实说白了很简单」）；每句允许 1~2 个语气词（提问句、警示句可到 2 个，让对话更有真人气）；但严禁「嗯、哦、对呀、是的呢、哎哟、好家伙、对喽、对吧对吧」等无意义堆砌，独白（单声）仍保持克制、靠停顿和语速变化；老张是实战派，说话干脆、直给、不拖长音；\n"
        ...
        "8. 声调起伏（重点新增）：问句收在升调（句尾用「呢/吗」或上扬疑问词如「怎么、吗、吧」）；警示句收在降调（用句号+短句，如「别抱侥幸心理。」）；重点词/结论词前后自然留白（用逗号、省略号「……」制造吸气停顿，如「金税四期一上线……个人卡流水，全看得见」）；长句中间微加速、句尾回落；整段避免每句同长同调，长短句交错；\n"
        "9. 不要输出任何解释、不要加标题，只输出改写后的对话稿本身。\n\n"
        "原稿：\n" + text
    )
    try:
...
def main():
    ...
    ap.add_argument("--female-rate", type=float, default=0.96, help="女声语速（默认0.96自然偏慢、提问不抢话、亲和）")
    ap.add_argument("--male-pitch", type=float, default=0.97, help="男声音调（默认0.97略沉稳但不闷、清晰可信）")
